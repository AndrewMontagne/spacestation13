...
/datum/brecipe/milkshake

	flour_amount = 1
	creates = "/obj/item/weapon/reagent_containers/food/snacks/liq/milkshake"



/obj/machinery/blender/New() // *** After making the recipe in defines\obj\food.dmi, add it in here! ***
	..()
...
obj/machinery/blender/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(src.broken > 0)
		if(src.broken == 2 && istype(O, /obj/item/weapon/screwdriver)) // If it's broken and they're using a screwdriver
			for(var/mob/V in viewers(src, null))
				V.show_message(text("\blue [user] starts to fix part of the blender."))
			sleep(20)
			for(var/mob/V in viewers(src, null))
				V.show_message(text("\blue [user] fixes part of the blender."))
			...
			sleep(20)
			for(var/mob/V in viewers(src, null))
				V.show_message(text("\blue [user] fixes the blender!"))
			src.icon_state = "mw"
			src.broken = 0 // Fix it!
		else
			user << "It's broken!"
	else if(src.dirty) // The blender is all dirty so can't be used!
		if(istype(O, /obj/item/weapon/cleaner)) // If they're trying to clean it then let them
			for(var/mob/V in viewers(src, null))
				V.show_message(text("\blue [user] starts to clean the blender."))
			sleep(20)
			for(var/mob/V in viewers(src, null))
				V.show_message(text("\blue [user] has cleaned the blender!"))
		...
		if(src.donkpocket_amount < 2)
			for(var/mob/V in viewers(src, null))
				V.show_message(text("\blue [user] adds a donk-pocket to the blender."))
		...
		if(!istype(extra_item, /obj/item)) //Allow one non food item to be added!
			...
			O.dropped(user)
			for(var/mob/V in viewers(src, null))
				V.show_message(text("\blue [user] adds [O] to the blender."))
		else
...
obj/machinery/blender/attack_paw(user as mob)
...
/obj/machinery/blender/attack_hand(user as mob) // The blender Menu
	var/dat
	if(src.broken > 0)
		dat = {"
<TT>Bzzzzttttt</TT>
...
"}
	...
	user << browse("<HEAD><TITLE>blender Controls</TITLE></HEAD><TT>[dat]</TT>", "window=blender")
	onclose(user, "blender")
	return



/***********************************
*   blender Menu Handling/Cooking
************************************/

/obj/machinery/blender/Topic(href, href_list)
	if(..())
		return
	...
	if(href_list["cook"])
		if(!src.operating)
			...
			if(operation == 1) // If cook was pressed
				for(var/mob/V in viewers(src, null))
					V.show_message(text("\blue The blender turns on."))
				...
				if(cooked_item == "") //Oops that wasn't a recipe dummy!!!
					if(src.egg_amount > 0 || src.flour_amount > 0 || src.water_amount > 0 || src.monkeymeat_amount > 0 || src.humanmeat_amount > 0 || src.donkpocket_amount > 0 && src.extra_item == null) //Make sure there's something inside though to dirty it
						...
						for(var/mob/V in viewers(src, null))
							V.show_message(text("\red The blender gets covered in muck!"))
						...
						for(var/mob/V in viewers(src, null))
							V.show_message(text("\red The blender breaks!")) //Let them know they're stupid
			...
			if(operation == 2) // If dispose was pressed, empty the blender
				src.egg_amount = 0
				src.flour_amount = 0
				src.water_amount = 0
				src.humanmeat_amount = 0
				...
				if(src.extra_item != null)
					src.extra_item.loc = get_turf(src) // Eject the extra item so important shit like the disk can't be destroyed in there
					src.extra_item = null
				usr << "You dispose of the blender contents."

			var/cooking = text2path(cooked_item) // Get the item that needs to be spanwed
			if(!isnull(cooking))
				for(var/mob/V in viewers(src, null))
					V.show_message(text("\blue The blender begins cooking something!"))
				...
				spawn(cook_time) //After the cooking time
					...
					src.operating = 0 // Turn the blender back off
					src.icon_state = "mw"
			else
				return